...
//TITULO DESVANECE
if(frameCount > 99 && frameCount < 130){ 
...
// 4 PROTAS.
if(frameCount > 159 && frameCount < 190){
   fill(10,140,150);
   text(" MARK HAMILL" ,150,150);
   text("CARRIE FISHER",450,150);
   text("HARRISON FORD", 150,300);
...
//4 PROTAS DESVANECE
if(frameCount > 189 && frameCount < 220){
...
//2 PROTAS.
if(frameCount > 249 && frameCount < 280){
  text("ALEC GUINNESS",150,200);
  text("DAVID PROWSE",450,200);
  textSize(20);
  text("Obi-Wan Kenobi",150,230);
...
//2 PROTAS DESVANECE.
if(frameCount > 279 && frameCount < 310){
...
//VOCES DARTH VADER
if(frameCount > 749 && frameCount < 780){
  fill(10,140,150);
  text("ANTHONY DANIELS, KENNY BAKER, PETER MAYHEW, \n DAVID PROWSE, JACK PURVIS, EDDIE BYRNE, \n JAMES EARL JONES",width/2,height/2);
  textSize(10);
  text("como la voz de Darth Vader",300,270);
...
if(frameCount > 779 && frameCount < 810){
...
//MUSICA
if(frameCount > 849 && frameCount < 880){
  fill(10,140,150);
  text("JOHN WILLIAMS", width/2, height/2);
  textSize(15);
  text("Musica compuesta por",width/2,170);
...
//MUSICA DESAPARECE
if(frameCount > 879 && frameCount < 910){
...
//PRODUCTOR
if(frameCount > 949 && frameCount < 980){
  fill(10,140,150);
  text("GARY KURTZ", width/2, height/2);
  textSize(15);
  text("Producida por",width/2,170);
...
if(frameCount > 979 && frameCount < 1010){
...
if(frameCount > 1049 && frameCount < 1080){
  fill(10,140,150);
  text("GEORGE LUCAS", width/2, height/2);
  textSize(15);
  text("Escrita y Dirigida por",width/2,170);
}
if(frameCount > 1079 && frameCount < 1110){
  float verde = map(frameCount, 1080, 1110, 140, 0);
  float azul = map(frameCount, 1080, 1110, 150, 0);
  float rojo = map(frameCount, 1080, 1110, 10, 0);
  fill(rojo,verde,azul);
  text("GEORGE LUCAS", width/2, height/2);
  textSize(15);
  text("Escrita y Dirigida por",width/2,170);
